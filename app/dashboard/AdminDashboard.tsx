...
export const AdminDashboard = () => {
    ...
    const [activeTab, setActiveTab] = useState<"members" | "courses" | "events">("members");
    ...
    const [courseForm, setCourseForm] = useState<{
        ...
        duration: string;
        imageFile: File | null;
    }>({ title: "", description: "", category: "", duration: "", imageFile: null });
    ...
    const submitCourse = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setCourseForm({ title: "", description: "", category: "", duration: "", imageFile: null });
    ...
    return (
        <div className="p-10">
            ...
            <div className="mb-6 flex gap-2">
                <button
                    className={`px-4 py-2 rounded ${activeTab === "members" ? "bg-black text-white" : "border"
                        }`}
                ...
                <button
                    className={`px-4 py-2 rounded ${activeTab === "courses" ? "bg-black text-white" : "border"
                        }`}
                ...
                <button
                    className={`px-4 py-2 rounded ${activeTab === "events" ? "bg-black text-white" : "border"
                        }`}
                ...
                <div>
                    {boardLoading ? (
                        <div className="flex flex-col items-center justify-center py-12">
                            ...
                            <p className="text-blue-800 text-lg font-medium">Loading members...</p>
                        </div>
                    ) : boardMembers.length === 0 ? (
                        <h2 className="text-center text-gray-600">No requests for board members</h2>
                    ) : (
            ...
            {activeTab === "courses" && (
                <div>
                    <h2 className="text-2xl font-semibold mb-4">{editingCourseId ? "Edit Course" : "Create Course"}</h2>
                    <form className="bg-white border rounded-xl p-6 mb-8" onSubmit={submitCourse}>
                    ...
                    {coursesLoading ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            {courses.map((c) => (
                                <div key={c._id} className="bg-white rounded-xl shadow p-4 border">
            ...
            {activeTab === "events" && (
                <div>
                    <h2 className="text-2xl font-semibold mb-4">{editingEventId ? "Edit Event" : "Create Event"}</h2>
                    <form className="bg-white border rounded-xl p-6 mb-8" onSubmit={submitEvent}>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <input
                                className="border rounded px-3 py-2"
                                placeholder="Name"
                            ...
                            <input
                                ...
                                accept="image/*"
                                className="border rounded px-3 py-2"
                                onChange={(e) => setEventForm({ ...eventForm, imageFile: e.target.files?.[0] || null })}
                            />
                            <input
                                className="border rounded px-3 py-2"
                                placeholder="PDF URL"
                    ...
                    {eventsLoading ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            {events.map((ev) => (
                                <div key={ev._id} className="bg-white rounded-xl shadow p-4 border">
                                    {ev.image && <img src={ev.image} alt={ev.name} className="w-full h-40 object-cover rounded mb-3" />}
